...
IN_OUT = {
    '../data/train.csv': ('../data/attempt10/pre-train.csv', True),
    '../data/test.csv': ('../data/attempt10/pre-test.csv', False)
}
...
def norm_text(text):
    ...
    words = [clean_word(w) for s in sent_tokenize(text) \
             for w in word_tokenize(handle_contract(s))]
