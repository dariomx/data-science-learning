...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]],
        ...
        target_gold_words_log_prob = torch.gather(P, index=target_padded[
                                                           1:].unsqueeze(-1),
                                                  dim=-1).squeeze(
            -1) * target_masks[1:]
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5,
                    max_decoding_time_step: int = 70) -> List[
        Hypothesis]:
        ...
        while len(
            ...
            contiuating_hyp_scores = (hyp_scores.unsqueeze(1).expand_as(
