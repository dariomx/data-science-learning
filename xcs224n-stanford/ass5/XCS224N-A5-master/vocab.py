...
class VocabEntry(object):
    ...
    def __init__(self, word2id=None):
        ...
        self.id2char = {v: k for k, v in
                        self.char2id.items()}  # Converts integers to characters
    ...
    def words2charindices(self, sents):
        """ Convert list of sentences of words into list of list of list of
        character indices.
        ...
        for in_sen in sents:
            out_sen = []
            for word in in_sen:
                out_word = [self.start_of_word]
                out_word += [self.char2id[c] for c in word]
                out_word += [self.end_of_word]
                out_sen.append(out_word)
            out_sents.append(out_sen)
        return out_sents
    ...
    def to_input_tensor_char(self, sents: List[List[str]],
        ...
        """ Convert list of sentences (words) into tensor with necessary
        padding for
        shorter sentences.
        ...
        max_word_length)
        """
        sents_var = pad_sents_char(self.words2charindices(sents),
                                   self.char2id['<pad>'])
        sents_var = torch.tensor(sents_var, device=device, dtype=torch.long)
    ...
    def from_corpus(corpus, size, freq_cutoff=2):
        ...
        @param corpus (list[str]): corpus of text produced by read_corpus
        function
        @param size (int): # of words in vocabulary
        @param freq_cutoff (int): if word occurs n < freq_cutoff times,
        drop the word
        @returns vocab_entry (VocabEntry): VocabEntry instance produced from
        provided corpus
        """
...
class Vocab(object):
    ...
    def build(src_sents, tgt_sents, vocab_size, freq_cutoff) -> 'Vocab':
        """ Build Vocabulary.
        @param src_sents (list[str]): Source sentences provided by
        read_corpus() function
        @param tgt_sents (list[str]): Target sentences provided by
        read_corpus() function
        @param vocab_size (int): Size of vocabulary for both source and
        target languages
        @param freq_cutoff (int): if word occurs n < freq_cutoff times,
        drop the word.
        """
