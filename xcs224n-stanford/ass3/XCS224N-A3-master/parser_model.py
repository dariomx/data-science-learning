...
class ParserModel(nn.Module):
    """ Feedforward neural network with an embedding layer and single hidden
    layer.
    ...
    PyTorch Notes:
        ...
        prefix. Thus,
            you should add the "self." prefix layers, values, etc. that you
            want to utilize
    ...
    def embedding_lookup(self, t):
        """ Utilize `self.pretrained_embeddings` to map input `t` from input
        tokens (integers)
            ...
            PyTorch Notes:
                ...
                features. Each feature is represented by an integer (input
                token).
                - In PyTorch the Embedding object,
                e.g. `self.pretrained_embeddings`, allows you to
                    go from an index to embedding. Please see the
                    documentation (
                    https://pytorch.org/docs/stable/nn.html#torch.nn.Embedding)
                    to learn how to use `self.pretrained_embeddings` to
                    extract the embeddings for your tensor `t`.
        ...
        x = self.pretrained_embeddings(t).view(batch_size,
                                               n_features * self.embed_size)
        return x

    def forward(self, t):
        """ Run the model forward.

            Note that we will not apply the softmax function here because it
            is included in the loss function nn.CrossEntropyLoss
            ...
            PyTorch Notes:
                ...
                function is applied to the tensor.
                    For example, if you created an instance of your
                    ParserModel and applied it to some `t` as follows,
        ...
        the layers of the network)
                                 without applying softmax (batch_size,
                                 n_classes)
        """
        x = self.embedding_lookup(t)
        h = nn.functional.relu(self.embed_to_hidden(x))
        return self.hidden_to_logits(self.dropout(h))
